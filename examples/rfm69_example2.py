"""
Basic example to query RFM69HW internal register information and receive all communications directed to specific node
"""
...
try:
    while True:
        packet = radio.receiveDone()
        if packet is None:
#             print("waiting for packet ...")
            time.sleep(0.1)
        else:
            timeElapsed = ((time.ticks_ms() - timeNow) / 1000)
            timeNow = time.ticks_ms()
